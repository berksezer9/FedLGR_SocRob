...
import pickle
import warnings
# Chained-job resume (OFFICEDB_MODIFICATIONS.md item 23):
import random
import numpy as np
import json
...
def _load_checkpoint_params(checkpoint_path, fallback):
    # Chained-job resume (OFFICEDB_MODIFICATIONS.md item 23): overrides a
...
def run_strategy(strategy, strategy_name, coeff, client_fn, clients, rounds, epochs, output, aug, ray_init_args,
    ...
    # Chained-job resume (OFFICEDB_MODIFICATIONS.md item 23): rounds_this_run
    ...
    if not chunked:
        ...
        # ---- Chained-job resume (OFFICEDB_MODIFICATIONS.md item 23): this job only
        ...
        if len(state["completed_tasks"]) == n_tasks:
            ...
            # OFFICEDB_MODIFICATIONS.md item 23 for the documented scope cut.
...
def run(args):
    ...
    # Chained-job resume (OFFICEDB_MODIFICATIONS.md item 23): main_fcl.py
    ...
    # Chained-job resume (OFFICEDB_MODIFICATIONS.md item 23): a fresh
                ...
                for strat_cl in strategies_cl:
                    ...
                    if strat_cl in ['EWC', 'EWCOnline', 'SI', 'MAS']:
                        ...
                        for coeff in reg_coefficients:
                            ...
                            # Chained-job resume (OFFICEDB_MODIFICATIONS.md item 23): _ckpt_path is
...
if __name__ == "__main__":
    ...
    parser.add_argument("--round_offset", type=int, default=0,
                         help="Added to Flower's server_round before task_idx/boundary math (chained-job "
                              "resume, OFFICEDB_MODIFICATIONS.md item 23). 0 (default) reproduces original "
                              "behavior exactly.")
    parser.add_argument("--rounds_this_run", type=int, default=None,
                         help="Actual num_rounds this process executes (default: falls back to --rounds, i.e. "
                              "the original one-shot behavior). See OFFICEDB_MODIFICATIONS.md item 23.")
